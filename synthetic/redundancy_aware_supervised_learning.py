...
class RedundancyAwareMMDL(nn.Module):
    ...
    def __init__(self, encoders, redundancy_predictors, redundancy_head, heads, fusion, head, has_padding=False):
        """Instantiate MMDL Module

        Args:
            encoders (List): List of nn.Module encoders, one per modality.
            redundancy_predictors (List): List of nn.Module encoders, one per modality.
            redundancy_head (List): List of nn.Module encoders, one per modality.
        ...
        self.redundancy_predictors = nn.ModuleList(redundancy_predictors)
        self.redundancy_head = redundancy_head
    ...
    def forward(self, inputs):
        ...
        latents_redundancies = []
        for i, outs_i in enumerate(outs):
            # Normalize each row (vector) to unit norm
            tmp = self.redundancy_predictors[i](outs_i.detach())
            latents_redundancies.append(tmp / tmp.norm(p=2, dim=1, keepdim=True))

        outs_redundancies = []
        for i, latents_i in enumerate(latents_redundancies):
            outs_redundancies.append(self.redundancy_head(latents_i.detach()))
        outs_redundancies.append((self.redundancy_head(latents_redundancies[0]) + self.redundancy_head(latents_redundancies[1])) / 2)

        return out_pred_joint, outs, outs_modalities, latents_redundancies, outs_redundancies

def supclip(c, s, y, tau=0.5):
    same_label = (y[:, None] == y[None, :]).float()
    distance_matrix = torch.cdist(c, s, p=2.0).pow(2)
    numerator = (distance_matrix * same_label).sum() / same_label.sum()
    denominator = distance_matrix.mul(-1/(2*tau)).exp().mean(1).log().mean()
...
def train(
        encoders, redundancy_predictors, redundancy_head, fusion, head, heads, train_dataloader, valid_dataloader,
    ...
    """
    if not pretrain:
        model = (RedundancyAwareMMDL(encoders, redundancy_predictors, redundancy_head, heads, fusion, head,
    ...
    def _trainprocess(best_model):
        ...
        for epoch in range(total_epochs):
            ...
            for j in train_dataloader:
                op.zero_grad()
                out_pred_joint, outs, outs_modalities, latents_redundancies, outs_redundancies = model([_processinput(i).to(device)
                                                                                      for i in j[:-1]])
                ...
                for out in outs_redundancies:
                    loss += deal_with_objective(
                        objective, out, j[-1], objective_args_dict)

                assert len(latents_redundancies) == 2, "Unicity Aware MMDL only works with 2 modalities"

                loss += 100 * supclip(latents_redundancies[0], latents_redundancies[1], j[-1])
                loss += 1000 * (latents_redundancies[0] - latents_redundancies[1]).norm(p=2, dim=1).pow(2).mean()
            ...
            with torch.no_grad():
                ...
                for j in valid_dataloader:
                    ...
                    # UNPACK MODEL OUTPUT
                    out_pred_joint, outs, outs_modalities, latents_redundancies, outs_redundancies = model(
...
def train_redundancy(
    ...
    def _trainredundancyprocess(best_model):
        op = optimtype(chain(
                              model.redundancy_predictors.parameters(),
                              model.redundancy_head.parameters()), lr=lr, weight_decay=weight_decay
                             )
        bestvalloss = 10000
        bestloss = 10000
        bestacc = 0
        ...
        for epoch in range(total_epochs):
            ...
            with torch.no_grad():
                ...
                for j in valid_dataloader:
                    ...
                    # LOSS ON REDUNDANCY PREDICTIONS
                    loss = 100 * supclip(latents_redundancies[0], latents_redundancies[1], j[-1])
                    loss += 1000 * (latents_redundancies[0] - latents_redundancies[1]).norm(p=2, dim=1).pow(2).mean()
                    for out in outs_redundancies:
                        loss += deal_with_objective(
    ...
    best_model = _trainredundancyprocess(best_model)
...
def single_test(
    ...
    with torch.no_grad():
        ...
        totalloss_redundancies = [0.0 for _ in range(2)]

        pred_joint = []
        true = []

        pred_modalities = [[] for _ in range(2)]
        pred_redundancies = [[] for _ in range(2)]
        ...
        for _, j in enumerate(test_dataloader):
            ...
            out_pred_joint, outs, outs_modalities, latents_redundancies, outs_redundancies = model(inputs)
            ...
            if isinstance(criterion, nn.CrossEntropyLoss):
                ...
                loss_joint = criterion(out_pred_joint, targets_ce)
            ...
            for idx, out_mod in enumerate(outs_modalities):
                if isinstance(criterion, nn.CrossEntropyLoss):
                    ...
                    loss_mod = criterion(softmax(out_mod), targets_ce)
                totalloss_modalities[idx] += loss_mod.item() * batch_size

            # ---------- REDUNDANCY LOSSES ----------
            for idx in range(len(outs_modalities)):
                if isinstance(criterion, nn.CrossEntropyLoss):
                    loss_red = traditional_cross_entropy_from_probs(softmax(outs_redundancies[idx]), targets_ce) # criterion(outs_redundancies[idx], targets_ce)
                else:
                    loss_red = criterion(outs_redundancies[idx], targets_ce)
                totalloss_redundancies[idx] += loss_red.item() * batch_size
            ...
            if task == "classification":
                ...
                for idx in range(len(outs_modalities)):
                    pred_redundancies[idx].append(torch.argmax(outs_redundancies[idx], dim=1))
        ...
        pred_redundancies = [torch.cat(p, 0) for p in pred_redundancies]

        # ---------- AVERAGE LOSSES ----------
        ce_joint = totalloss_joint / totals
        ce_modalities = [l / totals for l in totalloss_modalities]
        ce_redundancies = [l / totals for l in totalloss_redundancies]
        ...
        acc_redundancies = [accuracy(true, p) for p in pred_redundancies]

        # ---------- PRINT ----------
        print(f"Joint accuracy: {acc_joint:.4f}, CE: {ce_joint:.4f}")
        for idx, (acc, ce) in enumerate(zip(acc_modalities, ce_modalities)):
            print(f"Modality head {idx} accuracy: {acc:.4f}, CE: {ce:.4f}")
        for idx, (acc, ce) in enumerate(zip(acc_redundancies, ce_redundancies)):
            print(f"Redundancy head {idx} accuracy: {acc:.4f}, CE: {ce:.4f}")

        num_classes = torch.max(true) + 1

        redundancy = log(num_classes) - max(np.mean(ce_redundancies), ce_joint)
        unicity_1 = log(num_classes) - min(max(ce_modalities[0], ce_joint), max(np.mean(ce_redundancies), ce_joint)) - redundancy
        unicity_2 = log(num_classes) - min(max(ce_modalities[1], ce_joint), max(np.mean(ce_redundancies), ce_joint)) - redundancy
        synergy = log(num_classes) - ce_joint - redundancy - unicity_1 - unicity_2
