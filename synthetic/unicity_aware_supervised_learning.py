...
class UnicityAwareMMDL(nn.Module):
    ...
    def forward(self, inputs):
        ...
        latents_unicities = []
        for i, outs_i in enumerate(outs):
            latents_unicities.append(self.unicity_predictors[i](outs_i.detach()))

        outs_unicities = []
        for i, latents_i in enumerate(latents_unicities):
            outs_unicities.append(self.unicity_heads[i](latents_i.detach()))
...
def unicity_aware_train(
        encoders, unicity_predictors, unicity_heads, heads, fusion, head, train_dataloader, valid_dataloader,
    ...
    def _trainprocess():
        ...
        for epoch in range(total_epochs):
            ...
            for j in train_dataloader:
                ...
                loss += joint_entropy_loss(latents_unicities[0], outs[1].detach()) / 2
                loss += joint_entropy_loss(latents_unicities[1], outs[0].detach()) / 2
...
def single_test(model, test_dataloader, criterion=nn.CrossEntropyLoss(), task="classification", auprc=False,
                input_to_float=True, cluster=None, sub_acc=None, save_preds=None):
...
def test(
        model, test_dataloaders_all, dataset='default', method_name='My method', is_packed=False,
        criterion=nn.CrossEntropyLoss(), task="classification", auprc=False, input_to_float=True, no_robust=False,
        cluster=None, sub_acc=None, save_preds=None):
    """
    ...
    """
    if no_robust:
        return single_test(model, test_dataloaders_all, criterion, task, auprc, input_to_float, cluster=cluster,
                           sub_acc=sub_acc, save_preds=save_preds)
