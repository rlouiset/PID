...
from synthetic.redundancy_aware_supervised_learning import train, test, train_redundancy
...
if __name__ == "__main__":
    ...
    redundancy_predictors = [nn.Sequential(Linear(args.hidden_dim, 2*args.hidden_dim).to(device),
                                nn.ReLU(),
                                Linear(2*args.hidden_dim, args.hidden_dim).to(device))
                    for input_dim in input_dims]
    redundancy_head = MLP(args.n_latent, args.hidden_dim, args.num_classes).to(device) # Linear(args.hidden_dim, args.num_classes).to(device)
