...
from synthetic.unicity_aware_supervised_learning import train, test
...
with open("synthetic/experiments/DATA_redundancy_num_classes=4.pickle", "rb") as f:
    data = pickle.load(f)

print(data['train'].keys())
labels = data['train']['label']  # or whatever key
...
if __name__ == "__main__":
    ...
    unicity_predictors = [nn.Sequential(Linear(args.hidden_dim, 4*args.hidden_dim).to(device),
                                nn.ReLU(),
                                Linear(4*args.hidden_dim, args.hidden_dim).to(device))
                    for input_dim in input_dims]
    unicity_heads = [Linear(args.hidden_dim, args.num_classes).to(device) for input_dim in input_dims]
    heads = [Linear(args.hidden_dim, args.num_classes).to(device) for input_dim in input_dims]

    fusion = nn.Sequential(
        Concat(),
        MLP(len(args.modalities)*args.hidden_dim, args.n_latent, args.n_latent)
    ).to(device)
