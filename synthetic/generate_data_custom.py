...
for setting in dim_info:
    ...
    for _ in range(num_data):
        ...
        if setting != "synergy":
            ...
            label = int(label_prob * num_classes) - int(int(label_prob * num_classes) == num_classes)
            ...
            if num_classes == 4:
                ...
                label_prob_0 = 1 / (1 + math.exp(-np.mean(label_vector_0)))
                ...
                label_prob_1 = 1 / (1 + math.exp(-np.mean(label_vector_1)))

                label_prob_on = label_prob_0 * (1 - label_prob_1) + (1 - label_prob_0) * label_prob_1  # in [0,1]
                label_on = int(label_prob_on * num_classes)-1

                # OFF
                label_vector_0 = np.copy(np.array(raw_features['0'])[args.label_dim[0]:(args.label_dim[0]+args.label_dim[0])])
                label_vector_0 = label_transform_0(torch.Tensor(label_vector_0)).detach().numpy()
                label_prob_0 = 1 / (1 + math.exp(-np.mean(label_vector_0)))

                label_vector_1 = np.copy(np.array(raw_features['1'])[args.label_dim[0]:(args.label_dim[0]+args.label_dim[0])])
                label_vector_1 = label_transform_1(torch.Tensor(label_vector_1)).detach().numpy()
                label_prob_1 = 1 / (1 + math.exp(-np.mean(label_vector_1)))

                label_prob_off = label_prob_0 * (1 - label_prob_1) + (1 - label_prob_0) * label_prob_1  # in [0,1]
                label_off = int(label_prob_off*num_classes)-1

                label = 2*label_prob_off + label_prob_on
                label = min(label, num_classes - 1)

                dataset.append((label, modality_data))
            if num_classes == 2:
                label_vector_0 = np.copy(np.array(raw_features['0'])[:args.label_dim[0]])
                label_vector_0 = label_transform_0(torch.Tensor(label_vector_0)).detach().numpy()
                label_prob_0 = 1 / (1 + math.exp(-np.mean(label_vector_0)))
                ...
                label_prob = label_prob_0 * (1 - label_prob_1) + (1 - label_prob_0) * label_prob_1  # in [0,1]
                label = int(label_prob * num_classes) - int(int(label_prob * num_classes) == num_classes)

                dataset.append((label, modality_data))
            else:
                assert num_classes in [2, 4], "num_classes superior to 2 or 4 not defined yet"
    ...
    dict_of_total_labels[setting] = total_labels
