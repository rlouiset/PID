...
class MLP3(torch.nn.Module):
    ...
    def __init__(
        ...
        Args:
            indim (int): Input dimension
            hiddim (int): Hidden layer dimension (used for both hidden layers)
            outdim (int): Output dimension
            dropout (bool): Whether to apply dropout
            dropoutp (float): Dropout probability
            output_each_layer (bool): Whether to return intermediate outputs
        """
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.output_each_layer:
            return [0, x, h1, h2, self.lklu(out)]
