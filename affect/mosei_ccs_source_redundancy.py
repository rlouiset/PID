...
parser.add_argument("--data-path", default="/Users/robinlouiset/Downloads/mosei_senti_data.pkl", type=str)
...
# Feature dims per modality key (MOSEI: vision=35, audio=74, text=300)
...
def binarize(labels):
    """Sentiment >= 0 → positive (1), negative (0)."""
    return (labels >= 0).astype(np.int64)


def to_7class(labels):
    """Map [-3, 3] to 7 integer bins."""
...
class MOSEIDataset(Dataset):
    ...
    Applies augmentation during training: Gaussian noise + temporal dropout.
    """
...
def get_dataloaders(path, mod0_key="vision", mod1_key="text",
                    num_classes=2, batch_size=32, num_workers=4,
...
def compute_pid_global(joint_ce, mod0_ce, mod1_ce, red_ce, src_red_ce, num_classes, targets,
    ...
    print(f"R={i_r:.4f} ({100*ratio_src:.1f}% Source)  "
          f"U_video={i_u0:.4f}  U_text={i_u1:.4f}  S={i_s:.4f}  I={i_total:.4f}")
...
if __name__ == "__main__":
    ...
    print(f"Loading MOSEI  [{pair_name}]  dims=({dim0}, {dim1})")
    ...
    def split_stats(loader, name):
        ...
        print(f"  {name:<6s}  n={n:>6d}  [{class_str}]  H(Y)={h:.4f}  majority_acc={majority_acc:.4f}")
    ...
    heads = [
        MLP(args.hidden_dim, args.hidden_dim, args.num_classes).to(device),
        MLP(args.hidden_dim, args.hidden_dim, args.num_classes).to(device),
    ]

    # Joint fusion: concat → MLP3 → head
