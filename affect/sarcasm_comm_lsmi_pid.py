"""
LSMI-based pointwise PID estimation for MUsTARD with CoMM architecture.
  ...
  Note: RUS adjustment is applied only at distribution level (correction loop),
        not to the raw mean PID values.
...
class SarcasmDataset(Dataset):
    """Labels are ±1 in the pickle; remapped to 0/1. Augmentation: noise + temporal dropout."""
    def __init__(self, split_dict, mod0_key="vision", mod1_key="text",
                 augment=False, noise_std=0.1, temporal_dropout_max=0.4):
...
def get_dataloaders(path, mod0_key, mod1_key,
                    batch_size=16, num_workers=4, noise_std=0.1, temporal_dropout_max=0.4):
    ...
    train_clean_ds = SarcasmDataset(data["train"], mod0_key, mod1_key, augment=False)
    return (
