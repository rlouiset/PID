"""
CoMM/FactorCL architecture for MUsTARD sarcasm PID estimation.
  - Per-modality encoder : Conv1d projection → 5-layer Transformer (5 heads, 40-d, pre-norm)
  - Fusion               : CLS token + concat sequences → 1-layer Transformer (8 heads) → CLS
...
# MUsTARD shares feature dims with UR-FUNNY (same extraction pipeline)
MODALITY_DIMS = {"vision": 371, "audio": 81, "text": 300}
NUM_CLASSES   = 2  # binary: 0=not sarcastic, 1=sarcastic
...
def remap_labels(labels):
    """Map {-1, +1} → {0, 1}."""
    return ((np.array(labels) + 1) // 2).astype(np.int64)


class SarcasmDataset(Dataset):
    """
    MUsTARD dataset. Labels are ±1 in the pickle; remapped to 0/1.
    Augmentation: Gaussian noise + temporal dropout (CoMM/FactorCL strategy).
    """
    def __init__(self, split_dict, mod0_key="vision", mod1_key="text",
        ...
        self.labels = torch.from_numpy(remap_labels(split_dict["labels"]))
...
def get_dataloaders(path, mod0_key, mod1_key,
                    batch_size=16, num_workers=4, noise_std=0.1, temporal_dropout_max=0.8):
    with open(path, "rb") as f:
        data = pickle.load(f)
    train_ds = SarcasmDataset(data["train"], mod0_key, mod1_key,
                              augment=True, noise_std=noise_std,
                              temporal_dropout_max=temporal_dropout_max)
    valid_ds = SarcasmDataset(data["valid"], mod0_key, mod1_key, augment=False)
    test_ds  = SarcasmDataset(data["test"],  mod0_key, mod1_key, augment=False)
    return (
...
class CoMMEncoder(nn.Module):
    """
    Per-modality encoder: Conv1d(n_features → embed_dim) + optional sinusoidal pos-emb
    + 5-layer TransformerEncoder (5 heads, pre-norm). Returns (batch, T, embed_dim).
    """
...
class FusionTransformer(nn.Module):
    """CLS token + concat modality sequences → 1-layer self-attention → CLS output."""
...
def test_comm(model, testdata):
    ...
    return {
        "joint_acc":       (pred_joint.argmax(1) == true_labels).float().mean().item(),
        "modalities_acc": [(pred_m0.argmax(1) == true_labels).float().mean().item(),
                           (pred_m1.argmax(1) == true_labels).float().mean().item()],
...
def compute_pid_global(joint_ce, mod0_ce, mod1_ce, red_ce, src_red_ce,
                       targets, mod0_name="mod0", mod1_name="mod1"):
...
if __name__ == "__main__":
    ...
    print(f"Loading MUsTARD  [{pair_name}]  dims=({dim0}, {dim1})  embed={args.embed_dim}")
    ...
    def split_stats(loader, name):
        ...
        print(f"  {name:<6s}  n={n:>4d}  "
              f"[not sarcastic: {probs[0]:.3f}  sarcastic: {probs[1]:.3f}]  "
